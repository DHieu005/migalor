...
from cluster.node.entities import DiscoveryRequest, MoodResponse, TaskRequest, TaskResponse
...
class NodeClient:
    ...
    async def process_tasks(self, task_ids: list[str]) -> dict[str, str]:
        response = await self._client.post(
            "/tasks/process/",
            json=TaskRequest(task_ids=task_ids).dict(),
        )

        if response.status_code != 200:
            logger.warning(f"could not process tasks on ({self._node_ip}:{self._port}): {response.content}")
            return {}

        return TaskResponse(**response.json()).results
